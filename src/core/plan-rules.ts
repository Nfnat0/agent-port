...
import { contentHash, normalizeContent } from "./fingerprints.js";
import type {
  AdapterContext,
  AgentId,
  CanonicalAgentSetup,
  ...
  for (const component of source.components) {
    ...
    switch (component.kind) {
      ...
      case "rule":
        ...
        break;
      case "skill":
        changes.push(planApproximation(component, target.agent, "skills", "skill"));
        break;
      case "custom-agent":
        changes.push(planApproximation(component, target.agent, "agents", "custom agent"));
        break;
      case "command":
        changes.push(planApproximation(component, target.agent, "commands", "command"));
        break;
      case "hook":
        changes.push({
          type: "manual-review",
          ...
          detail: component.command ? `command: ${component.command}` : undefined,
...
export async function generateFilesForTarget(
  ...
  for (const change of changes.filter((item) => item.target === target.agent)) {
    ...
    if (component.kind === "instruction" && capabilities.instruction && change.type !== "skip") {
      files.push(
        generatedFile(
          target.agent,
          displayPath(capabilities.instruction.path, context.cwd, context.homeDir),
    ...
    if (component.kind === "rule" && change.type !== "skip") {
      const rulePath =
        target.agent === "cursor"
          ? path.join(".cursor", "rules", "agent-port-generated.mdc")
...
function planApproximation(
  ...
  return {
    ...
    path: generatedPath(".agent-port/generated", target, category, component.name),
    componentId: component.id,
    risk: component.risk,
    portability: "approximate",
  };
...
function renderCursorRule(rule: CanonicalRule | CanonicalInstruction): string {
  ...
  return [
    "---",
    "description: Generated by agent-port from portable agent instructions",
    "alwaysApply: true",
    "---",
    "",
    content.trim(),
